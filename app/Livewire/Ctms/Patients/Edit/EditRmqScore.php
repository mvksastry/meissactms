...
class EditRmqScore extends Component
{
    ...
    public function fnEditRMQInfo()
    {
        ...
        try {
            ...
            if ($result) {        
                $msg = 'RMQ Score For Patient ['.$name.'] update successfull!'; 
                LivewireAlert::title('Patient RMQ info updated')->success()->asToast()->show(); 
                $this->comSuccess = $msg;
                Log::channel('patient')->info($msg);
            } else {
                $msg = 'RMQ Score For Patient ['.$name.'] could not be saved';
                LivewireAlert::title('Patient RMQ info failed')->warning()->asToast()->show();
                $this->sysAlertDanger = $msg;
                Log::channel('patient')->info($msg);
            }
        } catch (QueryException $e) {
            // Handles database-related errors (e.g., duplicate email)
            $msg = 'Database error: RMQ Score For patient ['.$name.'] while saving : '.$e->getMessage();
            LivewireAlert::title('Patient RMQ info failed')->warning()->asToast()->show();
            Log::channel('patient')->info($msg);
            $this->sysAlertDanger = $msg;
        } catch (\Exception $e) {
            // Handles any other general exceptions
            $msg = 'Unexpected error: RMQ Score For Patient ['.$name.'] while saving : '.$e->getMessage();
            LivewireAlert::title('Patient RMQ info failed')->warning()->asToast()->show();
            Log::channel('patient')->info($msg);
            $this->sysAlertDanger = $msg;
        } 
