...
class EditMdtreInfo extends Component
{
    ...
    public function fnEditMDTREInfo()
    {
        ...
        try {
            ...
            if ($result) {        
                $msg = 'MDTRE Info For Patient ['.$name.'] update successfull!';  
                LivewireAlert::title('Patient M&DTRE info updated')->success()->asToast()->show();
                //$this->comSuccess = $msg;
                Log::channel('patient')->info($msg);
            } else {
                $msg = 'MDTRE Info For Patient ['.$name.'] could not be saved';
                LivewireAlert::title('Patient M&DTRE info failed')->warning()->asToast()->show();
                $this->sysAlertDanger = $msg;
                Log::channel('patient')->info($msg);
            }
        } catch (QueryException $e) {
            // Handles database-related errors (e.g., duplicate email)
            $msg = 'Database error: MDTRE Info For Patient ['.$name.'] while saving : '.$e->getMessage();
            LivewireAlert::title('Patient M&DTRE info failed')->warning()->asToast()->show();
            Log::channel('patient')->info($msg);
            $this->sysAlertDanger = $msg;
        } catch (\Exception $e) {
            // Handles any other general exceptions
            $msg = 'Unexpected error: MDTRE Info For Patient ['.$name.'] while saving : '.$e->getMessage();
            LivewireAlert::title('Patient M&DTRE info failed')->warning()->asToast()->show();
            Log::channel('patient')->info($msg);
            $this->sysAlertDanger = $msg;
        } 
