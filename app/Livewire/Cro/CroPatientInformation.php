...
class CroPatientInformation extends Component
{
    ...
    public function render()
    {
		...
		{
            $this->activePatients = $this->getAllEnrolledActivePatients();
            if(count($this->activePatients) <= 0)
            {
                $this->search_result = "Patients with status 'Active' Not Found";
                LivewireAlert::title($this->search_result)->info()->show();
            }  
        }
        return view('livewire.cro.cro-patient-information');
    }
