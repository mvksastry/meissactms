<div>
 ...
 <!-- Content Wrapper. Contains page content -->
  <div class="content-wrapper">
                  ...
                  <tbody> 
                    @foreach($enrolledPatients as $row)
                      <tr class="{{ $rowSelected === $row->patient_uuid ? 'table-secondary' : '' }}">
                        ...
                        <td>
                            <button wire:click="selectedPatient('{{ $row->patient_uuid}}')" class="btn btn-block btn-primary rounded" type="button" ><i class="ion ion-person"></i>&nbsp VIEW DATA</button>
                        </td>
                      </tr>
                    @endforeach
                  </tbody>
            ...
            @if($patientInfoButtons)
              <div class="row">
                <div class="col-sm-3 col-md-2">
                  <button wire:click="fnShowPrimaryInfo('{{ $patient_uuid }}')" type="button" class="btn btn-block btn-light"><i class="ion ion-person"></i>&nbsp Primary Infos</button>
                </div>
                <!-- /.col -->
                <div class="col-sm-3 col-md-2">
    ...
    <!-- Main content -->
    @if($p1)
      <livewire:ctms.datareview.datarev-personal-info :patient_uuid="$patient_uuid" : data_type="$data_type" />
    @endif

    @if($p2)
      <livewire:ctms.datareview.datarev-life-style :data_type="$data_type" :patient_uuid="$patient_uuid" :entry="$entry" />
    @endif
    
    @if($p3)
      <livewire:ctms.datareview.datarev-clinibiochem-info :data_type="$data_type" :patient_uuid="$patient_uuid" :entry="$entry" />
    @endif

    @if($p4)
      <livewire:ctms.datareview.datarev-sensory-exams :data_type="$data_type" :patient_uuid="$patient_uuid" :entry="$entry" />
    @endif

    @if($p5)
      <livewire:ctms.datareview.datarev-mdtr-exams :data_type="$data_type" :patient_uuid="$patient_uuid" :entry="$entry" />
    @endif

    @if($p6)
      <livewire:ctms.datareview.datarev-reports :data_type="$data_type" :patient_uuid="$patient_uuid" :entry="$entry" />
    @endif

    @if($p7)
      <livewire:ctms.datareview.datarev-pfirman-grades :data_type="$data_type" :patient_uuid="$patient_uuid" :entry="$entry" />
    @endif

    @if($p8)
      <livewire:ctms.datareview.datarev-visual-analog-scores :data_type="$data_type" :patient_uuid="$patient_uuid" :entry="$entry" />
    @endif

    @if($p9) 
      <livewire:ctms.datareview.datarev-modq-scores :data_type="$data_type" :patient_uuid="$patient_uuid" :entry="$entry" />
    @endif
    
    @if($p10)
      <livewire:ctms.datareview.datarev-rmq-scores :data_type="$data_type" :patient_uuid="$patient_uuid" :entry="$entry" />
    @endif
